// src/screens/GestionUsuarios.js
...
export default function GestionUsuarios() {
    const { user, userClaims, loading } = useContext(AuthContext); // Obtener userClaims del contexto
    const isAdmin = userClaims?.admin; // Verificar si el usuario actual es admin

    const [users, setUsers] = useState([]);
    const [loadingUsers, setLoadingUsers] = useState(true);
    const [newUserEmail, setNewUserEmail] = useState('');
    const [newUserPassword, setNewUserPassword] = useState('');
    const [assignAdminToNewUser, setAssignAdminToNewUser] = useState(false); // Checkbox para admin en nuevo usuario

    useEffect(() => {
        if (!loading && isAdmin) { // Solo si ya cargó y es admin
            fetchUsers();
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            setUsers(result.data.users); // Asume que la función devuelve { users: [...] }
        } catch (error) {
    ...
    const handleCreateUser = async () => {
        ...
        try {
            ...
            const result = await createUserFunction({
                ...
                role: assignAdminToNewUser ? 'admin' : 'commonUser' // Envía el rol deseado
            });
    ...
    const handleUpdateUserRole = async (uid, currentClaims, newAdminStatus) => {
        // Lógica para actualizar el rol de un usuario existente
        const newClaims = {
            ...currentClaims, // Mantener claims existentes si los hubiera
            admin: newAdminStatus,
            commonUser: !newAdminStatus // Si es admin, no es commonUser y viceversa (simple)
        };
        // Puedes refinar esta lógica de roles
        if (newAdminStatus) {
            delete newClaims.commonUser; // Elimina el commonUser si se convierte en admin
        } else {
            newClaims.commonUser = true; // Asegúrate de que commonUser sea true si no es admin
        }
    ...
    // Renderizado del componente (ejemplo)
    return (
        <View style={styles.container}>
            ...
            {/* Sección para Listar y Gestionar Usuarios Existentes */}
            <View style={styles.card}>
